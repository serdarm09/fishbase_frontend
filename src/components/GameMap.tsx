...
const GameMap: React.FC<GameMapProps> = ({
  ...
  const statusMessage = playerBoat
    ...
    : 'Drop anchor on any blue tile to start earning fishing XP.';
    ...
    <div className="w-full max-w-5xl mx-auto space-y-4">
      ...
      <div className="ocean-card bg-blue-50/70 border space-y-2">
        <p className="text-blue-800 text-sm flex items-start gap-2">
          ...
          <span>
            Shift your boat at least once every 24 hours. Waiting longer reduces that day&apos;s XP to 10%.
            Occupied tiles belong to other captains, so look for open waters when you redeploy.
          </span>
        </p>
        {playerBoost && playerBoost.level !== 'NONE' && playerBoost.image && (
          <div className="flex items-center gap-2 text-xs text-gray-500">
            ...
            <span>
              {playerBoost.name || 'Active boost'} grants an extra {Math.round(playerBoost.multiplier * 100)}% XP while you cruise.
            </span>
          </div>
        )}
      </div>
